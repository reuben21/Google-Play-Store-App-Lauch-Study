...
def cancel():
    mcanvas.delete("all")
    head = Label(mcanvas, text="Google \nPlayStore \n App launch \nStudy", width=30, font=("Lucida", 50, 'bold'),
                 fg='#ffffff', bg='#102131')
...
def saveing(x, y, z, p):
    ...
    if z == 'C:\\InternshipFinal\\App-data.csv':
        try:
            ...
            insert_query = "INSERT INTO appdata (appname,category,rating,review,size,install,type,price,cont_rat,genres,last_updated,current_version,android_version) VALUES('" + \
                           value[0] + "', '" + value[1] + "', '" + value[2] + "', '" + value[3] + "', '" + value[
                               4] + "', '" + value[5] + "', '" + value[6] + "', '" + value[7] + "', '" + value[
                               8] + "', '" + value[9] + "', '" + date + "', '" + value[10] + "', '" + value[
                               11] + "' );"  # queries for inserting values
        ...
        try:
            connection = pymysql.connect(host="localhost", user="root", password="",
                                         database="googleplaystore")  # database connection
            cursor = connection.cursor()
            insert_query = "INSERT INTO addreview (app,trans_rev,sentiment,sent_polar,sent_subj) VALUES('" + value[
        ...
        if (isinstance(float(x[3].get()), float) and isinstance(float(x[4].get()), float)):
            if x[2].get() == 'Neutral':
                if float(x[3].get()) == 0 and 1 >= float(x[4].get()) >= 0:
                    ...
                    tk.messagebox.showwarning('Neutral sentiment',
                                              'Please provide a 0 in Sentiment polarity and Sentiment Subjectivity.')
                    return True
            elif x[2].get() == 'Positive':
                if float(x[3].get()) > 0 and 1 >= float(x[4].get()) >= 0:
                    d.append(False)
                else:
                    tk.messagebox.showwarning('Positive sentiment',
                                              'Please provide a positive value in Sentiment polarity and Sentiment Subjectivity.')
                    return True
                ...
                if float(x[3].get()) < 0 and 1 >= float(x[4].get()) >= 0:
                    ...
                    tk.messagebox.showwarning('Positive sentiment',
                                              'Please provide a negative value in Sentiment polarity and non negative value in Sentiment Subjectivity.')
                    return True
    except:
        tk.messagebox.showwarning('Wrong Value',
                                  'Please provide a float value in Sentiment polarity and Sentiment Subjectivity.')
        return True
...
def add_rev():
    ...
    txt2 = []
    for i in range(1, 6):
        tk.Label(val, text=header2[i - 1], width=17, font=("Lucida", 11, 'italic'), fg='#ffffff', bg='#102131').grid(
...
def check(x, z):
    ...
    try:
        if (
        isinstance(float(x[2].get()), float)):  # code for checking the user entered a valid rating in the entry field
...
def add_app_data():
    ...
    dateco = Frame(val)
    for i in range(1, 14):
        Label(val, text=header[i - 1], width=11, font=("Lucida", 11, 'italic'), fg='white', bg='#102131').grid(row=i,
                                                                                                               column=0,
                                                                                                               padx=2,
                                                                                                               pady=2)
    for i in range(1, 14):
...
def login():
    ...
    Label(val, text="", bg='#102131', width='100', height='17').place(x=45,
                                                                      y=120)  # blue background in middle of window
    ...
    Label(val, text="", bg='#102131').pack()  # for leaving a space in between
    Button(val, text="LOGIN", bg="black", width=15, height=1, font=("Open Sans", 13, 'bold'), fg='white',
           command=login_verify).pack()
    Label(val, text="", bg='#102131').pack()  # for leaving a space in between
    Button(val, text="New User? Register Here", height="2", width="30", bg='black', font=("Open Sans", 10, 'bold'),
...
def register_user():
    ...
    if fullname.get() and email.get() and password.get() and repassword.get() and gender.get():  # checking for all empty values in entry field
        if (len(phone.get()) != 10) and int(phone.get()):  # checking for selection of university
            ph_no = Label(mcanvas, text="Enter the Valid Phone Number", fg="red", font=("Lucida", 11), width='30',
                          anchor=W, bg='white')
                ...
                if re.match("^.+@(\[?)[a-zA-Z0-9-.]+.([a-zA-Z]{2,3}|[0-9]{1,3})(]?)$",
                    ...
                    if password.get() == repassword.get():  # checking both password match or not
                        ...
                        try:
                            ...
                            connection.commit()  # commiting the connection then closing it.
                            connection.close()  # closing the connection of the database
                            rs = Label(mcanvas, text="Registration Sucess", fg="green", font=("Lucida", 11), width='30',
                                       anchor=W, bg='white')

                            mcanvas.create_window(200, 480, window=rs)  # printing successful registration message
                            pl = Button(mcanvas, text='Proceed to Login ->', width=20, font=("Open Sans", 9, 'bold'),
                                        bg='brown', fg='white', command=login)
                            mcanvas.create_window(500, 480, window=pl)  # button to navigate back to login page
                        except pymysql.Error:
                            tk.messagebox.showinfo('Error', "Database Error")
                    else:
                        ps = Label(mcanvas, text="Password does not match", fg="red", font=("Lucida", 11), width='30',
                                   anchor=W, bg='white')
                        mcanvas.create_window(200, 480, window=ps)
                        return
                else:
                    pvi = Label(mcanvas, text="Please enter valid email id", fg="red", font=("Lucida", 11), width='30',
                                anchor=W, bg='white')
                    mcanvas.create_window(200, 480, window=pvi)
                    return
            else:
                pat = Label(mcanvas, text="Please accept the agreement", fg="red", font=("Lucida", 11), width='30',
                            anchor=W, bg='white')
                mcanvas.create_window(200, 480, window=pat)
                return
    else:
        pfi = Label(mcanvas, text="Please fill all the details", fg="red", font=("Lucida", 11), width='30', anchor=W,
                    bg='white')
...
def register():
    ...
    entry_4.pack()
    Checkbutton(val, text="I accept all terms and conditions", variable=tnc, bg='#102131',
                font=("Open Sans", 9, 'bold'), fg='brown').pack()
    Button(val, text='Submit', width=20, font=("Open Sans", 13, 'bold'), bg='black', fg='white',
...
def revv():
    ...
    list_of_apps_most_positive_sentiments = (data[(data.App == appli[filtered.index(search.get())]) & (
                data.Sentiment == 'Positive')].Translated_Review).tolist()
    #     print(list_of_apps_most_positive_sentiments)
    list_of_apps_most_negative_sentiments = (data[(data.App == appli[filtered.index(search.get())]) & (
                data.Sentiment == 'Negative')].Translated_Review).tolist()
    #     print(list_of_apps_most_negative_sentiments)
    list_of_apps_most_average_sentiments = (data[(data.App == appli[filtered.index(search.get())]) & (
                data.Sentiment == 'Neutral')].Translated_Review).tolist()
    ...
    if (len(list_of_apps_most_positive_sentiments) > len(list_of_apps_most_negative_sentiments)) and (
            len(list_of_apps_most_positive_sentiments) > len(list_of_apps_most_average_sentiments)):
        Label(canvas[0], text='User liked this app', width=25, anchor=CENTER,
        ...
        Label(canvas[0], text='User disliked this app', width=25, anchor=CENTER,
              font=("Helvetica", 15, 'bold', 'italic')).pack()
    else:
        Label(canvas[0], text='User neither liked nor disliked this app', width=25, anchor=CENTER,
              font=("Helvetica", 15, 'bold', 'italic')).pack()
...
def Update_cat():
    ...
    try:
        ...
        for i in range(len(list2)):
            insert_query = "INSERT INTO catupdate (Categories,Downloads) VALUES('" + list2[i] + "', '" + str(
                list1[i]) + "' );"  # queries for inserting values
            cursor.execute(insert_query)  # executing the queries
        connection.commit()  # commiting the connection then closing it.
        connection.close()
...
def category():
    ...
    q1 = Button(mcanvas, text="The percentage download in each category in the playstore.", width=70, height=2,
                font=("Lucida", 13, 'bold'), fg='#ffffff', bg='black', command=fn.functq1)
    ...
    q3 = Button(mcanvas, text="""Which category of apps have managed to get the most,
                least and an average of 2,50,000 downloads atleast.""", width=70, height=2, font=("Lucida", 13, 'bold'),
                fg='#ffffff', bg='black', command=fn.functq3)
    #    q4.bind("<Button-1>", function_q4)
    #    q4.place(x=40,y=120)
    mcanvas.create_window(375, 200, window=q3)

    q0 = Button(mcanvas, text="The percentage of Apps in  each category in the playstore .", width=70, height=2,
                font=("Lucida", 13, 'bold'), fg='#ffffff', bg='black', command=fn.functq0)
    ...
    q6 = Button(mcanvas, text="""For the years 2016,2017,2018 what are the category of apps that have got the 
                most and the least downloads""", width=70, height=2, font=("Lucida", 13, 'bold'), fg='#ffffff',
                bg='black', command=fn.functq6)
    #    q5.bind("<Button-1>", function_q5)  
...
def nextc1():
    ...
    q8 = Button(mcanvas, text="""Amongst Sports, Entertainment,social,News,Events,Travel and Game,
                which is the category of app that is most likely to be downloaded""", width=70, height=2,
                font=("Lucida", 13, 'bold'), fg='#ffffff', bg='black', command=fn.functq8)
    #    q3.bind("<Button-1>", function_q3)
...
c) More than 30 mb""", width=90, height=6, font=("Lucida", 10, 'bold'), fg='#ffffff', bg='black', command=fn.functq5)
    ...
    q10_1 = Button(mcanvas, text="Month with maximum downloads for each of the category.", width=72, height=2,
                   font=("Lucida", 13, 'bold'), fg='#ffffff', bg='black', command=funct10)
    #    q3.bind("<Button-1>", function_q3)
...
def nexti1():
    ...
    #    q=mcanvas.create_rectangle(40,40,500,80,fill='black')
    q10_2 = Button(mcanvas, text="Ratio of downloads for the App that qualifies as teen vs mature17+", width=70,
                   height=2, font=("Lucida", 13, 'bold'), fg='#ffffff', bg='black', command=fn.functq10_2)
    #    q3.bind("<Button-1>", function_q3)
    mcanvas.create_window(375, 120, window=q10_2)

    q11 = Button(mcanvas,
                 text="Which quarter of which year has generated the highest number of install for each app used",
                 width=72, height=2, font=("Lucida", 12, 'bold'), fg='#ffffff', bg='black', command=fn.question11)
    #    q3.bind("<Button-1>", function_q3)
...
def rrev():
    ...
    q12 = Button(mcanvas, text="""Which app has manage to generate the most positive, negative 
                 sentiments and generated approximately the same ratio""", width=70, height=2,
                 font=("Lucida", 13, 'bold'), fg='#ffffff', bg='black', command=twelve)
    #    q3.bind("<Button-1>", function_q3)
    ...
    q13 = Button(mcanvas, text="""the relation between the sentiment-polarity and sentiment-subjective,
                 the sentiment subjectivity for a sentiment polarity of 0.4""", width=70, height=2,
                 font=("Lucida", 13, 'bold'), fg='#ffffff', bg='black', command=fn.function_q13)
    #    q3.bind("<Button-1>", function_q3)
    mcanvas.create_window(375, 200, window=q13)
    #    q=mcanvas.create_rectangle(40,40,500,80,fill='black')
    q14_15 = Button(mcanvas, text="""Positive, negative and neutral reviews of an app,
                 does the user like these app""", width=70, height=2, font=("Lucida", 13, 'bold'), fg='#ffffff',
                    bg='black', command=fourteen)
    #    q3.bind("<Button-1>", function_q3)
...
def app():
    ...
    q7 = Button(mcanvas, text="""All those apps,whose android version is not an issue and can 
                work with varying devices.""", width=70, height=2, font=("Lucida", 13, 'bold'), fg='#ffffff',
                bg='black', command=fn.functq7)
    #    q3.bind("<Button-1>", function_q3)
    mcanvas.create_window(375, 100, window=q7)

    q7_2 = Button(mcanvas, text="What is the percentage increase or decrease in the downloads.", width=70, height=2,
                  font=("Lucida", 13, 'bold'), fg='#ffffff', bg='black', command=fn.functq7_2)
    #    q3.bind("<Button-1>", function_q3)
    mcanvas.create_window(375, 180, window=q7_2)

    q4 = Button(mcanvas, text="The apps that managed to get the highest maximum rating from the user.", width=70,
                height=2, font=("Lucida", 13, 'bold'), fg='#ffffff', bg='black', command=fn.functq4)
    #    q5.bind("<Button-1>", function_q5)
    mcanvas.create_window(375, 260, window=q4)
    q9 = Button(mcanvas, text="""App managed to get get over 1,00,000 downloads, 
                and managed to get an average rating of 4.1 and above.""", width=70, height=2,
                font=("Lucida", 13, 'bold'), fg='#ffffff', bg='black', command=fn.functq9)
    #    q5.bind("<Button-1>", function_q5)
    ...
    q16 = Button(mcanvas, text="""Which month of the year, is the best indicator to the average 
                 downloads that an app will generate over the entire year .""", width=70, height=2,
                 font=("Lucida", 13, 'bold'), fg='#ffffff', bg='black', command=funct16)
    #    q5.bind("<Button-1>", function_q5)
...
def nexta1():
    ...
    #    q=mcanvas.create_rectangle(40,40,500,80,fill='black')
    q17 = Button(mcanvas, text="Does the size of the app influence the number of install that it get?", width=70,
                 height=2, font=("Lucida", 12, 'bold'), fg='#ffffff', bg='black', command=fn.functq17)
    #    q3.bind("<Button-1>", function_q3)
...
mcanvas.create_window(400, 200, window=head)
# =====================================================options==================================================
lbl_over = Button(root, text="Add Data", width=25, height='2', font=("Lucida", 13, 'italic'), fg='#ffffff', bg='black',
                  command=login)
# lbl_over.bind("<Button-1>")
lbl_over.place(x=8, y=220)

lbl_category = Button(root, text="Category", width=25, height='2', font=("Lucida", 13, 'italic'), fg='#ffffff',
                      bg='black', command=category)
# lbl_category.bind("<Button-1>")
lbl_category.place(x=8, y=220 + 60)

lbl_Installs = Button(root, text="Installs", width=25, height='2', font=("Lucida", 13, 'italic'), fg='#ffffff',
                      bg='black', command=install)
# lbl_Installs.bind("<Button-1>")
lbl_Installs.place(x=8, y=220 + 60 + 60)

lbl_searchapp = Button(root, text="Search App", width=25, height='2', font=("Lucida", 13, 'italic'), fg='#ffffff',
                       bg='black', command=fn.searchapp)
# lbl_searchapp.bind("<Button-1>")
lbl_searchapp.place(x=8, y=220 + 60 + 120)

lbl_machine = Button(root, text="App Info", width=25, height='2', font=("Lucida", 13, 'italic'), fg='#ffffff',
                     bg='black', command=app)
lbl_machine.bind("<Button-1>")
lbl_machine.place(x=8, y=220 + 60 + 120 + 60)

lbl_review = Button(root, text="Reviews", width=25, height='2', font=("Lucida", 13, 'italic'), fg='#ffffff', bg='black',
                    command=rrev)
# lbl_review.bind("<Button-1>")
lbl_review.place(x=8, y=220 + 60 + 120 + 120)

lbl_lastupdate = Button(root, text="Add Reviews", width=25, height='2', font=("Lucida", 13, 'italic'), fg='#ffffff',
                        bg='black', command=add_rev)
...
Button(rcanvas, text="Update The Installs\n Per Category", width=25, height='2', font=("Lucida", 13, 'italic'),
       fg='#ffffff', bg='black', command=Update_cat).place(x=35, y=220)
Button(rcanvas, text="Refresh main screen", width=25, height='2', font=("Lucida", 13, 'italic'), fg='#ffffff',
       bg='black', command=cancel).place(x=30, y=120)
# ======================================bottom canvas============================
...
# ==================================================group name==============================================
name = Label(root, text="Ctrl+Alt+Del", width=15, height=1, font=("Helvetica", 15, 'bold', 'italic'), fg='#ffffff',
             bg='#102131')
